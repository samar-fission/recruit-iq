...
# This script is expected to live inside jd_desired_experience_and_education/

# Config
REGION=${AWS_REGION:-us-east-1}
FUNCTION_NAME=${FUNCTION_NAME:-jd-desired-experience-education}
...
APP_PKG_DIR="$APP_SRC/jd_desired_experience_and_education"
mkdir -p "$APP_PKG_DIR"
cp "$WORKDIR/__init__.py" "$APP_PKG_DIR/"
cp "$WORKDIR/"*.py "$APP_PKG_DIR/"

cd "$APP_SRC" && zip -qr "$BUILD_DIR/app.zip" jd_desired_experience_and_education && cd "$PROJECT_ROOT"
...
if aws lambda get-function --function-name "$FUNCTION_NAME" --region "$REGION" >/dev/null 2>&1; then
  ...
  aws lambda update-function-configuration \
    --function-name "$FUNCTION_NAME" \
    --runtime "$PY_VERSION" \
    --handler jd_desired_experience_and_education.handler.handler \
    --layers "$LAYER_ARN" \
...
echo "Done. Test with:\naws lambda invoke --function-name $FUNCTION_NAME --region $REGION --cli-binary-format raw-in-base64-out --payload '{\"title\":\"Technical Lead – Java\",\"jd\":\"<paste JD>\",\"must_have_skills\":[\"Java\",\"Microservices\"]}' out.json && cat out.json && echo"
