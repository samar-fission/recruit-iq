...
FUNCTION_NAME=${FUNCTION_NAME:-jd-resume-desired-experience-scorer}
...
APP_SRC="$BUILD_DIR/app_src"; APP_PKG_DIR="$APP_SRC/resume_desired_experience_scorer"
mkdir -p "$APP_PKG_DIR"
cp "$WORKDIR/__init__.py" "$APP_PKG_DIR/" || true
cp "$WORKDIR/"*.py "$APP_PKG_DIR/"
cd "$APP_SRC" && zip -qr "$BUILD_DIR/app.zip" resume_desired_experience_scorer && cd "$PROJECT_ROOT"
...
if aws lambda get-function --function-name "$FUNCTION_NAME" --region "$REGION" >/dev/null 2>&1; then
  ...
  aws lambda update-function-configuration --function-name "$FUNCTION_NAME" --runtime "$PY_VERSION" --handler resume_desired_experience_scorer.handler.handler --layers "$LAYER_ARN" --environment "Variables={BEDROCK_REGION=$BEDROCK_REGION,BEDROCK_MODEL_ID=$BEDROCK_MODEL_ID,LOG_LEVEL=$LOG_LEVEL}" --region "$REGION" >/dev/null
else
  aws lambda create-function --function-name "$FUNCTION_NAME" --runtime "$PY_VERSION" --handler resume_desired_experience_scorer.handler.handler --zip-file "fileb://$BUILD_DIR/app.zip" --role "$ROLE_ARN" --layers "$LAYER_ARN" --environment "Variables={BEDROCK_REGION=$BEDROCK_REGION,BEDROCK_MODEL_ID=$BEDROCK_MODEL_ID,LOG_LEVEL=$LOG_LEVEL}" --timeout 30 --memory-size 512 --region "$REGION" >/dev/null
fi

echo "Done. Test with: aws lambda invoke --function-name $FUNCTION_NAME --region $REGION --cli-binary-format raw-in-base64-out --payload '{\"resume_text\":\"<resume>\",\"desired_experience\":[\"Led development of Java services\"]}' out.json && cat out.json && echo"
