...
# This script is expected to live inside jd_education_validator/

# Config
REGION=${AWS_REGION:-us-east-1}
FUNCTION_NAME=${FUNCTION_NAME:-jd-education-validator}
...
APP_PKG_DIR="$APP_SRC/jd_education_validator"
mkdir -p "$APP_PKG_DIR"
cp "$WORKDIR/__init__.py" "$APP_PKG_DIR/"
cp "$WORKDIR/"*.py "$APP_PKG_DIR/"

cd "$APP_SRC" && zip -qr "$BUILD_DIR/app.zip" jd_education_validator && cd "$PROJECT_ROOT"
...
if aws lambda get-function --function-name "$FUNCTION_NAME" --region "$REGION" >/dev/null 2>&1; then
  ...
  aws lambda update-function-configuration \
    --function-name "$FUNCTION_NAME" \
    --runtime "$PY_VERSION" \
    --handler jd_education_validator.handler.handler \
    --layers "$LAYER_ARN" \
...
echo "Done. Test with:\naws lambda invoke --function-name $FUNCTION_NAME --region $REGION --cli-binary-format raw-in-base64-out --payload '{\"education_preference\":[{\"education\":\"Bachelor\'s degree ...\",\"source_section\":\"Qualifications\"}]}' out.json && cat out.json && echo"
