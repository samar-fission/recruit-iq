...
# This script is expected to live inside tools/resume_skills_scorer/

REGION=${AWS_REGION:-us-east-1}
FUNCTION_NAME=${FUNCTION_NAME:-jd-resume-skills-scorer}
...
APP_PKG_DIR="$APP_SRC/resume_skills_scorer"
mkdir -p "$APP_PKG_DIR"
cp "$WORKDIR/__init__.py" "$APP_PKG_DIR/" || true
cp "$WORKDIR/"*.py "$APP_PKG_DIR/"

cd "$APP_SRC" && zip -qr "$BUILD_DIR/app.zip" resume_skills_scorer && cd "$PROJECT_ROOT"
...
if aws lambda get-function --function-name "$FUNCTION_NAME" --region "$REGION" >/dev/null 2>&1; then
  ...
  aws lambda update-function-configuration \
    ...
    --runtime "$PY_VERSION" \
    --handler resume_skills_scorer.handler.handler \
    --layers "$LAYER_ARN" \
...
echo "Done. Test with:\naws lambda invoke --function-name $FUNCTION_NAME --region $REGION --cli-binary-format raw-in-base64-out --payload '{\"resume_text\":\"<resume>\",\"skills_with_context\":[{\"skill\":\"Java\",\"jd_context\":\"Backend services\"}]}' out.json && cat out.json && echo"
