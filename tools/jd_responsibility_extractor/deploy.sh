...
# This script is expected to live inside jd_responsibility_extractor/

# Config
REGION=${AWS_REGION:-us-east-1}
FUNCTION_NAME=${FUNCTION_NAME:-jd-responsibility-extractor}
...
APP_PKG_DIR="$APP_SRC/jd_responsibility_extractor"
mkdir -p "$APP_PKG_DIR"
cp "$WORKDIR/__init__.py" "$APP_PKG_DIR/"
cp "$WORKDIR/"*.py "$APP_PKG_DIR/"

cd "$APP_SRC" && zip -qr "$BUILD_DIR/app.zip" jd_responsibility_extractor && cd "$PROJECT_ROOT"
...
if aws lambda get-function --function-name "$FUNCTION_NAME" --region "$REGION" >/dev/null 2>&1; then
  ...
  aws lambda update-function-configuration \
    --function-name "$FUNCTION_NAME" \
    --runtime "$PY_VERSION" \
    --handler jd_responsibility_extractor.handler.handler \
    --layers "$LAYER_ARN" \
...
echo "Done. Test with:\naws lambda invoke --function-name $FUNCTION_NAME --region $REGION --cli-binary-format raw-in-base64-out --payload '{\"title\":\"Senior Backend Engineer – FinTech\",\"years_of_experience\":\"8–10 years\",\"seniority_level\":\"Senior\",\"job_page\":\"<paste job page text>\"}' out.json && cat out.json && echo"
